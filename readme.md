...
| `DB_CONNECTION_STRING`      | DB connection string (**required** — the manager exits at startup if neither this nor `MONGODB_CONNECTION_STRING` is set; no hardcoded fallback). Supports MongoDB (`mongodb://`) and CouchDB (`http://`/`https://`). For local development use `mongodb://localhost:27017/intercom-manager` |
